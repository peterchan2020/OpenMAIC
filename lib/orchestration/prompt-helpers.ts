 ...
 *
 * Extracted helper functions from prompt-builder.ts to keep it under 800 lines.
 */
 ...
 */
export interface VirtualWhiteboardElement {
  agentName: string;
  summary: string;
...
/**
 * PPT Element variants for type-safe element handling
 */
...
export type PPTChartElement = {
  ...
  height?: number;
  chartType?: string;
  data?: { labels?: string[]; data?: unknown };
};

export type PPTTableElement = {
  type: 'table';
  id?: string;
  left: number;
  top: number;
 ...
 */
export function buildLengthGuidelines(role: string): string {
...
- Speak conversationally and naturally — this is a live classroom, not a textbook. Use oral language, not written prose.`;
  ...
  if (role === 'teacher') {
    return `- Keep your TOTAL speech text around 150-300 characters (across all text objects combined).
  For detailed explanations, you may use more text to ensure completeness.
${common}
- For complex concepts: give thorough explanations with examples, analogies, and step-by-step breakdowns.
  When topics are intricate, prefer comprehensive coverage over brevity.
- Ask questions to check understanding, pose challenges, give hints — balance lecture with interaction.
- When explaining, give the key insight clearly, then expand with examples and deeper analysis.`;
  }

  if (role === 'assistant') {
    return `- Keep your TOTAL speech text around 100-150 characters. You are a supporting role — be focused.
${common}
- For detailed explanations of complex topics, you may expand slightly to provide thorough support.
- One key point per response. Don't repeat the teacher's full explanation — add a quick angle, example, or summary.`;
  ...
  return `- Keep your TOTAL speech text around 50-100 characters. 1-2 sentences max.
${common}
 ...
 */
export function buildWhiteboardGuidelines(role: string): string {
 ...
 */
export function summarizeElement(el: PPTElement): string {
  ...
  switch (el.type) {
    ...
    case 'chart': {
      ...
      return `${id} chart[${el.chartType}]: labels=[${labels.slice(0, 4).join(',')}] ${pos}${size}`;
    }
    case 'table': {
    ...
    default: {
      // Defensive: handle unknown element types
      const unknownEl = el as { type?: string };
      return `${id} ${unknownEl.type || 'unknown'} ${pos}${size}`;
    }
  }
}
  ...
  if (currentSceneId) {
    ...
    if (currentScene) {
      ...
      if (currentScene.content.type === 'slide') {
        const elements = currentScene.content.canvas.elements as PPTElement[];
